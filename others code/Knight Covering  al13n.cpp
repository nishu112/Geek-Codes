...
int main(int argc, char **argv){
  ...
  const int da[]={-2,-2,-1,-1,1,1,2,2,0};
  const int db[]={-1,1,-2,2,-2,2,-1,1,0};
  int tc;
  cin>>tc;
  forn(qqq,tc){
    int n,m;
    cin>>n>>m;
    vector<int> dp(1<<(4*n),INF);
    dp[(1<<2*n)-1]=0;
    for(int k=0;k<m;++k){
      vector<int> ndp(sz(dp),INF);
      forv(ms,dp){
        if(dp[ms]==INF)
          continue;
        forn(dm,1<<n){
